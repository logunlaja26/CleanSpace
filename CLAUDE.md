...
# Open Xcode workspace (ALWAYS use .xcworkspace, NOT .xcodeproj)
...
# Run on specific physical device (REQUIRED for photo library access)
...
- **Device APIs:** @react-native-camera-roll/camera-roll, PhotoKit, AVFoundation
- **Monetization:** RevenueCat (react-native-purchases) or react-native-iap
...
**Goal:** Get a working React Native iOS project with all dependencies installed.
...
2. **Install All Dependencies**
   ```bash
   # Core libraries
   npm install react-native-sqlite-storage @react-native-camera-roll/camera-roll
   npm install react-native-permissions react-native-fs
   npm install @supabase/supabase-js react-native-device-info
   npm install react-native-background-fetch @tanstack/react-query
...
3. **Configure iOS Project in Xcode**
   - Open `ios/PhotoVideoCleaner.xcworkspace` in Xcode
   - Set up code signing (Signing & Capabilities → select your team)
...
4. **Set Up NativeWind/Tailwind**
   - Configure `tailwind.config.js`
   - Set up `babel.config.js` for NativeWind
